...
bool ComboAutoSelect(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiInputTextCallback text_cb, ImGuiComboFlags flags = ImGuiComboFlags_None);
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
bool ComboAutoSelect(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ImGuiInputTextCallback text_cb, ImGuiComboFlags flags = ImGuiComboFlags_None);
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
...
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
bool ComboAutoSelectEX(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ComboAutoSelectSearchCallback<T2> autoselect_callback, ImGuiInputTextCallback text_cb, ImGuiComboFlags flags);
template<typename T1, typename T2, typename = std::enable_if<std::is_convertible<T1, T2>::value>::type>
...
bool ComboAutoSelect(
	const char* combo_label, 
	int& selected_item, 
	const T1& items, 
	ComboItemGetterCallback<T2> item_getter, 
	ComboAutoSelectSearchCallback<T2> autoselect_callback,
	ImGuiInputTextCallback text_cb,
	ImGuiComboFlags flags
) {
	ImGui::BeginDisabled(Internal::IsContainerEmpty(items));
	bool ret = Internal::ComboAutoSelectEX(combo_label, selected_item, items, item_getter, autoselect_callback, text_cb, flags);
	ImGui::EndDisabled();
...
template<typename T1, typename T2, typename>
bool ComboAutoSelect(const char* combo_label, int& selected_item, const T1& items, ComboItemGetterCallback<T2> item_getter, ImGuiInputTextCallback text_cb, ImGuiComboFlags flags)
{
	return ComboAutoSelect(combo_label, selected_item, items, item_getter, Internal::DefaultComboAutoSelectSearchCallback, text_cb, flags);
}
...
template<typename T1, typename T2, typename>
bool ComboAutoSelectEX(
	const char* combo_label, 
	int& selected_item, 
	const T1& items, 
	ComboItemGetterCallback<T2> item_getter, 
	ComboAutoSelectSearchCallback<T2> autoselect_callback, 
	ImGuiInputTextCallback text_callback,
	ImGuiComboFlags flags
) {
	...
	const bool buffer_changed = InputTextEx(
		"##inputText", NULL, 
		combo_data->InputText, 
		ComboData::StringCapacity, 
		ImVec2(0, 0), 
		ImGuiInputTextFlags_AutoSelectAll | ImGuiInputTextFlags_::ImGuiInputTextFlags_CallbackEdit,
		// Added my own callback to get updates from when text changes
		text_callback, 
		NULL);
	PopStyleColor(2);
	PopStyleVar(1);
	...
	if (BeginListBox(listbox_name, ImVec2(items_max_width, CalcComboItemHeight(popup_item_count, 1.25f)))) {
		...
		// Commented these out as they cause a read violation
		// After some debugging, I am not willing to invest the time into resolving this issue atm
